...
rex_sql_table::get(rex::getTable('consent_manager_cookie'))
    ...
    ->ensureColumn(new rex_sql_column('variant', 'varchar(255)', true, ''))
...
$sql = rex_sql::factory();
$sql->setQuery('UPDATE ' . rex::getTable('consent_manager_text') . ' SET `text` = ? WHERE `uid` = ? AND `text` = ?', ['Details', 'toggle_details', 'Details anzeigen/ausblenden']);
$sql->setQuery('UPDATE ' . rex::getTable('consent_manager_text') . ' SET `text` = ? WHERE `uid` = ? AND `text` = ?', ['Details', 'toggle_details', 'Show/hide details']);
$sql->setQuery('UPDATE ' . rex::getTable('consent_manager_text') . ' SET `text` = ? WHERE `uid` = ? AND `text` = ?', ['Detaljer', 'toggle_details', 'Visa/dölj detaljer']);
